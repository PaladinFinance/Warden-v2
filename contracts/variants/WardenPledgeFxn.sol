...
/// @author Paladin
/*
    Delegation market (Pledge version) based on veFXN VotingEscrowDelegation contract
*/
contract WardenPledgeFxn is Owner, Pausable, ReentrancyGuard {
    ...
    IVotingEscrowBoost public immutable delegationBoost;
    ...
    */
    constructor(
        ...
        delegationBoost = IVotingEscrowBoost(_delegationBoost);
    ...
    */
    function pledgePercent(uint256 pledgeId, uint256 percent, uint256 endTimestamp) external nonReentrant whenNotPaused {
        ...
        uint256 amount = (delegationBoost.delegableBalance(msg.sender) * percent) / MAX_PCT;
    ...
    */
    function _pledge(uint256 pledgeId, address user, uint256 amount, uint256 endTimestamp) internal {
        ...
        IVotingEscrowBoost _delegationBoost = delegationBoost;
        _delegationBoost.checkpoint(user);
        if(_delegationBoost.allowance(user, address(this)) < amount) revert Errors.InsufficientAllowance();
        if(_delegationBoost.delegableBalance(user) < amount) revert Errors.CannotDelegate();

        // Check that this will not go over the Pledge target of votes
        if(_delegationBoost.adjustedVeBalance(pledgeParams.receiver) + amount > pledgeParams.targetVotes) revert Errors.TargetVotesOverflow();

        // Creates the DelegationBoost
        _delegationBoost.boostFrom(
            user,
