...
import { WardenFxn } from "../../typechain/variants/WardenFxn";
import { WardenFxnMultiBuy } from "../../typechain/variants/WardenFxnMultiBuy";
...
import { IVotingEscrowBoostTest } from "../../typechain/tests/IVotingEscrowBoostTest";
import { IVotingEscrowBoostTest__factory } from "../../typechain/factories/tests/IVotingEscrowBoostTest__factory";
...
let constants_path = "../utils/fxn-constant"
...
describe('WardenFxn MultiBuy contract tests - veFXN version', () => {
    ...
    let warden: WardenFxn
    let multiBuy: WardenFxnMultiBuy

    let baseToken: IERC20
    let veToken: IVotingEscrow
    let delegationBoost: IVotingEscrowBoostTest
    ...
    before(async () => {
        ...
        wardenFactory = await ethers.getContractFactory("WardenFxn");
        multiBuyFactory = await ethers.getContractFactory("WardenFxnMultiBuy");
        ...
        delegationBoost = IVotingEscrowBoostTest__factory.connect(BOOST_DELEGATION_ADDRESS, provider);
    ...
    beforeEach(async () => {
        ...
        const baseToken_amount = ethers.utils.parseEther('800');
        const lock_amount = ethers.utils.parseEther('20'); //change the lock amounts
        ...
        multiBuy = (await multiBuyFactory.connect(admin).deploy(
            ...
            delegationBoost.address,
            warden.address
        )) as WardenFxnMultiBuy;
        await multiBuy.deployed();


        //await getERC20(admin, BIG_HOLDER, baseToken, delegator.address, baseToken_amount);
        await getERC20(admin, "0x331174A9067e864A61B2F87861CCf006eD3bC95D", baseToken, admin.address, ethers.utils.parseEther('900'));
        await getERC20(admin, "0x69873461B8F29F3a3D35449e2173040e0AA1571d", baseToken, admin.address, ethers.utils.parseEther('900'));
        await getERC20(admin, "0xe0d102f9b8c5eE35428b2e855AdD000Bcf9c9636", baseToken, admin.address, ethers.utils.parseEther('900'));
        await getERC20(admin, "0xb84dfdD51d18B1613432bfaE91dfcC48899D4151", baseToken, admin.address, ethers.utils.parseEther('500'));

        
            //split between all delegators
        await baseToken.connect(admin).transfer(delegator1.address, ethers.utils.parseEther('2'));
        await baseToken.connect(admin).transfer(delegator2.address, ethers.utils.parseEther('3.500'));
        await baseToken.connect(admin).transfer(delegator3.address, ethers.utils.parseEther('2.750'));
        await baseToken.connect(admin).transfer(delegator4.address, ethers.utils.parseEther('2.500'));
        await baseToken.connect(admin).transfer(delegator5.address, ethers.utils.parseEther('1'));
        await baseToken.connect(admin).transfer(delegator6.address, ethers.utils.parseEther('1.500'));
        await baseToken.connect(admin).transfer(delegator7.address, ethers.utils.parseEther('5'));
        await baseToken.connect(admin).transfer(delegator8.address, ethers.utils.parseEther('1.750'));

        await baseToken.connect(delegator1).approve(veToken.address, ethers.utils.parseEther('2'));
        await baseToken.connect(delegator2).approve(veToken.address, ethers.utils.parseEther('3.500'));
        await baseToken.connect(delegator3).approve(veToken.address, ethers.utils.parseEther('2.750'));
        await baseToken.connect(delegator4).approve(veToken.address, ethers.utils.parseEther('2.500'));
        await baseToken.connect(delegator5).approve(veToken.address, ethers.utils.parseEther('1'));
        await baseToken.connect(delegator6).approve(veToken.address, ethers.utils.parseEther('1.500'));
        await baseToken.connect(delegator7).approve(veToken.address, ethers.utils.parseEther('5'));
        await baseToken.connect(delegator8).approve(veToken.address, ethers.utils.parseEther('1.750'));
        ...
        await veToken.connect(delegator1).create_lock(ethers.utils.parseEther('2'), lock_time);
        await veToken.connect(delegator2).create_lock(ethers.utils.parseEther('3.500'), lock_time);
        await veToken.connect(delegator3).create_lock(ethers.utils.parseEther('2.750'), lock_time);
        await veToken.connect(delegator4).create_lock(ethers.utils.parseEther('2.500'), lock_time);
        await veToken.connect(delegator5).create_lock(ethers.utils.parseEther('1'), one_week_lock_time);
        await veToken.connect(delegator6).create_lock(ethers.utils.parseEther('1.500'), lock_time);
        await veToken.connect(delegator7).create_lock(ethers.utils.parseEther('5'), lock_time);
        await veToken.connect(delegator8).create_lock(ethers.utils.parseEther('1.750'), lock_time);

        await baseToken.connect(admin).transfer(receiver.address, baseToken_amount.sub(lock_amount).sub(ethers.utils.parseEther('1')));
        await baseToken.connect(admin).transfer(receiver2.address, ethers.utils.parseEther('1'));
    ...
    describe('simpleMultiBuy', async () => {
        ...
        const bigger_amount = ethers.utils.parseEther('20')
        
        const max_price = price_per_vote2

        const fee_amount = amount.mul(max_price).mul(one_week.mul(duration + 1)).div(unit)
        const incorrect_fee_amount = amount.mul(max_price.div(5)).mul(one_week.mul(duration + 1)).div(unit)
        const bigger_fee_amount = bigger_amount.mul(max_price).mul(one_week.mul(duration + 1)).div(unit)

        const accepted_slippage = 10 // 0.1 %

        const minRequiredAmount = BigNumber.from(0)
        const bigger_minRequiredAmount = ethers.utils.parseEther('2')
        ...
        it(' should buy Boosts to cover requested amount + Event', async () => {
            ...
            for(let e of events){
                ...
                if(boost_index.toNumber() != expected_offers_indexes_order[expected_offers_indexes_order.length - 1]) {
                    let delegator_balance = await veToken.balanceOf(e.delegator, { blockTag: tx_block })
                    expect(e.amount).to.be.eq(
                        delegator_balance.sub(delegator_balance.mul(
                            BigNumber.from(BPS).sub(delegator_offer.maxPerc)
                        ).div(BPS))
                    )
                }

                let exact_boost_amount = await delegationBoost.delegatedBalance(boost_delegator, { blockTag: tx_block })
            ...
            const veToken_adjusted_receiver = await delegationBoost.adjustedVeBalance(receiver.address, { blockTag: tx_block })
        ...
        it(' should skip Offers with price over the maxPrice given', async () => {
            ...
            const other_amount = ethers.utils.parseEther('6')

            const lower_max_price = price_per_vote3
            const low_fee_amount = other_amount.mul(lower_max_price).mul(one_week.mul(duration + 1)).div(unit)
        ...
        it(' should return unused fee tokens to the buyer', async () => {
            ...
            const other_amount = ethers.utils.parseEther('6')
            const other_fee_amount = other_amount.mul(max_price).mul(one_week.mul(duration + 1)).div(unit)
        ...
        it(' should take all available amount if Boosts already taken on Offers', async () => {
            ...
            await baseToken.connect(receiver2).approve(warden.address, ethers.constants.MaxUint256)

            const fee_amount1 = await warden.estimateFeesPercent(delegator1.address, boost_buy_percent, duration)
            const fee_amount2 = await warden.estimateFeesPercent(delegator2.address, boost_buy_percent, duration)

            await warden.connect(receiver2).buyDelegationBoostPercent(delegator1.address, receiver2.address, boost_buy_percent, duration, fee_amount1)
            await warden.connect(receiver2).buyDelegationBoostPercent(delegator2.address, receiver2.address, boost_buy_percent, duration, fee_amount2)
            ...
            for(let e of events){
                ...
                && [1,2].includes(boost_index.toNumber())){
                    ...
                    let boost_percent = e.amount.mul(BPS).div(await veToken.balanceOf(e.delegator, { blockTag: tx_block }))
        ...
        it(' should skip Offer where delegator removed WardenFxn as Operator', async () => {
        ...
        it(' should skip Offers where lock is already over', async () => {
            ...
            const slightly_bigger_amount = ethers.utils.parseEther('11.5')
            const slightly_bigger_fee_amount = slightly_bigger_amount.mul(max_price).mul(one_week.mul(duration + 1)).div(unit)
    ...
    describe('preSortedMultiBuy', async () => {
        ...
        describe('other multiBuy tests', async () => {
            ...
            it(' should skip Offers with price over the maxPrice given', async () => {
                ...
                const other_amount = ethers.utils.parseEther('5.5')
            ...
            it(' should take all available amount if Boosts already taken on Offers', async () => {
                ...
                const fee_amount1 = await warden.estimateFeesPercent(delegator7.address, boost_buy_percent, duration)
                const fee_amount2 = await warden.estimateFeesPercent(delegator8.address, boost_buy_percent, duration)
    
                await warden.connect(receiver2).buyDelegationBoostPercent(delegator7.address, receiver2.address, boost_buy_percent, duration, fee_amount1)
                await warden.connect(receiver2).buyDelegationBoostPercent(delegator8.address, receiver2.address, boost_buy_percent, duration, fee_amount2)

    
                //Expected buy percent on Offers with already a Boost
                let boosts_expected_percent_buy: {[key: number]: number} = {}
                boosts_expected_percent_buy[7] = (await warden.offers(7)).maxPerc - boost_buy_percent
                boosts_expected_percent_buy[8] = (await warden.offers(8)).maxPerc - boost_buy_percent
                ...
                for(let e of events){
                    ...
                    // Check that it used the max % available for that Offer (except for the last one)
                    if(boost_index.toNumber() != expected_offers_indexes_order[expected_offers_indexes_order.length - 1] && [7,8].includes(boost_index.toNumber())){
                        let expected_percent = BigNumber.from(boosts_expected_percent_buy[boost_index.toNumber()])
                        let boost_percent = e.amount.mul(BPS).div(await veToken.balanceOf(e.delegator, { blockTag: tx_block }))
                        expect(boost_percent).to.be.closeTo(expected_percent, 1)
                    }
            ...
            it(' should skip Offer where delegator removed WardenFxn as Operator', async () => {
                ...
                const slightly_smaller_amount = ethers.utils.parseEther('8')
            ...
            it(' should skip Offers where lock is already over', async () => {
                ...
                const slightly_bigger_amount = ethers.utils.parseEther('7.5')
                const slightly_bigger_fee_amount = slightly_bigger_amount.mul(max_price).mul(one_week.mul(duration + 1)).div(unit)
    ...
    describe('sortingMultiBuy', async () => {
        ...
        const amount = ethers.utils.parseEther('7.5')
    ...
    describe('common', async () => {
        ...
        const amount = ethers.utils.parseEther('5.250')
