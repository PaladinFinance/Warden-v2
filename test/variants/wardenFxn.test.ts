...
describe('WardenFxn contract tests - veFXN version', () => {
    ...
    beforeEach(async () => {
        ...
        await getERC20(admin, "0x331174A9067e864A61B2F87861CCf006eD3bC95D", baseToken, delegator.address, ethers.utils.parseEther('900'));
        await getERC20(admin, "0x69873461B8F29F3a3D35449e2173040e0AA1571d", baseToken, delegator.address, ethers.utils.parseEther('900'));
        await getERC20(admin, "0xe0d102f9b8c5eE35428b2e855AdD000Bcf9c9636", baseToken, delegator.address, ethers.utils.parseEther('900'));
        await getERC20(admin, "0xb84dfdD51d18B1613432bfaE91dfcC48899D4151", baseToken, delegator.address, ethers.utils.parseEther('500'));
    ...
    describe('canDelegate', async () => {
        ...
        it(' should return true if the amount fits the Offer & the delegators balance - with active boost', async () => {
            ...
            await advanceTime(WEEK.mul(2).toNumber())
            await delegationBoost.connect(delegator).checkpoint(delegator.address)
        ...
        it(' should return false if the delegator did not approve WardenFxn', async () => {
    ...
    describe('buyDelegationBoost', async () => {
        ...
        it(' should create a Boost from the delegator to the caller', async () => {
            ...
            const veToken_adjusted_delegator = await delegationBoost.adjustedVeBalance(delegator.address, { blockTag: tx_block })
            ...
            expect(await delegationBoost.receivedBalance(receiver.address, { blockTag: tx_block })).to.be.closeTo(buy_amount, 1e8)
            expect(await delegationBoost.delegatedBalance(delegator.address, { blockTag: tx_block })).to.be.closeTo(buy_amount, 1e8)
        ...
        it(' should fail if wanted delegator did not approve WardenFxn', async () => {
    ...
    describe('buyDelegationBoostPercent', async () => {
        ...
        it(' should create a Boost from the delegator to the caller', async () => {
            ...
            expect(await delegationBoost.receivedBalance(receiver.address, { blockTag: tx_block })).to.be.closeTo(estimated_buy_amount, 1e8)
            expect(await delegationBoost.delegatedBalance(delegator.address, { blockTag: tx_block })).to.be.closeTo(estimated_buy_amount, 1e8)
