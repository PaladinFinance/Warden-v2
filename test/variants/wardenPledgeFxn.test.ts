...
import { WardenPledgeFxn } from "../../typechain/variants/WardenPledgeFxn";
...
import { IVotingEscrowBoost } from "../../typechain/interfaces/IVotingEscrowBoost";
import { IVotingEscrowBoost__factory } from "../../typechain/factories/interfaces/IVotingEscrowBoost__factory";
...
let constants_path = "../utils/fxn-constant" // by default: veToken
...
const min_vote_diff = ethers.utils.parseEther('1')
...
describe('Warden Pledge contract tests - veFXN version', () => {
    let admin: SignerWithAddress
    let chest: SignerWithAddress
    let receiver: SignerWithAddress
    let externalUser: SignerWithAddress

    let wardenPledge: WardenPledgeFxn
    ...
    let delegationBoost: IVotingEscrowBoost
    ...
    const baseToken_amount = ethers.utils.parseEther('2000');
    const lock_amounts = [
    ...
    before(async () => {
        ...
        wardenPledgeFactory = await ethers.getContractFactory("WardenPledgeFxn");
        ...
        delegationBoost = IVotingEscrowBoost__factory.connect(BOOST_DELEGATION_ADDRESS, provider);
    ...
    describe('createPledge', async () => {
        ...
        const target_votes = ethers.utils.parseEther("750")
        ...
        const target_votes2 = ethers.utils.parseEther("1200")
        ...
        it(' should calculate the correct needed amount of votes if the receiver already has veToken balance', async () => {
            ...
            const lock_amount = ethers.utils.parseEther('125')
    ...
    describe('pledge', async () => {
        ...
        const deleg_amount1 = ethers.utils.parseEther("350")
        ...
        it(' should delegate the correct amount for the given duration to the correct address (& emit Event) ', async () => {
            ...
            expect(await delegationBoost.delegatedBalance(delegator1.address, { blockTag: tx_block })).to.be.eq(boost_bias)
            expect(await delegationBoost.receivedBalance(receiver.address, { blockTag: tx_block })).to.be.eq(boost_bias)
            ...
            await delegationBoost.connect(delegator1).checkpoint(delegator1.address)
        ...
        it(' should allow another user to to get rewards from the same Pledge', async () => {
            ...
            const deleg_amount2 = ethers.utils.parseEther("275")
            ...
            const tx_block = (await pledge_tx).blockNumber
            expect(await delegationBoost.delegatedBalance(delegator2.address, { blockTag: tx_block })).to.be.eq(boost_bias)
            ...
            await delegationBoost.connect(delegator1).checkpoint(delegator1.address)
            await delegationBoost.connect(delegator2).checkpoint(delegator2.address)
        ...
        it(' should allow the same user to get rewards from another Pledge', async () => {
            ...
            const deleg_amount2 = ethers.utils.parseEther("150")
        ...
        it(' should fail if trying to delegate more than the Pledge target amount', async () => {
            ...
            const deleg_amount2 = ethers.utils.parseEther("525")
    ...
    describe('pledgePercent', async () => {
        ...
        it(' should delegate the all delegable balance if given 100%', async () => {
            ...
            const previous_boost_received = await delegationBoost.receivedBalance(delegator2.address, { blockTag: tx_block })
            const expected_boost_amount = (await veToken.balanceOf(delegator1.address, { blockTag: tx_block })).sub(previous_boost_received)

            const boost_slope = expected_boost_amount.div(boost_duration)
            const boost_bias = boost_duration.mul(boost_slope)

            expect(await delegationBoost.delegatedBalance(delegator1.address, { blockTag: tx_block })).to.be.eq(boost_bias.add(previous_boost_received))
            expect(await delegationBoost.receivedBalance(receiver.address, { blockTag: tx_block })).to.be.eq(boost_bias)
    ...
    describe('closePledge', async () => {
        ...
        it(' should return all non-distributed rewards to the given addresse (& emit Event)', async () => {
            ...
            const deleg_amount2 = ethers.utils.parseEther("200")
            const boost_week_duration2 = BigNumber.from(2)
    ...
    describe('updateMinVoteDiff', async () => {
        ...
        const new_min_vote_diff = ethers.utils.parseEther('150')
